...
struct segtree{
	...
	int atleast_helper(int value, int x, int l, int r){
		if(r-l==1){
			if(values[x].val >= value) return l;
			else return -1;
		}
		int m = (l+r)/2;
		if(values[2*x+1].val >= value)
			return atleast_helper(value,2*x+1,l,m);
		else
			return atleast_helper(value,2*x+2,m,r);
	}

	int atleast(long long int val){
		return atleast_helper(val,0,0,size);
	}
};
