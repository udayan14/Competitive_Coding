...
struct segtree{
	...
	item NEUTRAL_ELEMENT = {LLONG_MIN,0};
	...
	int atleast_helper(int value, int l_bound, int x, int l, int r){
		if(r-l==1){
			if(values[x].val >= value) return l;
			else return -1;
		}
		int m = (l+r)/2;
		if(l_bound >= m)
			return atleast_helper(value,l_bound,2*x+2,m,r);
		else{
			int ret = atleast_helper(value,l_bound,2*x+1,l,m);
			if(ret != -1) return ret;
			return atleast_helper(value,l_bound,2*x+2,m,r);
		}
	}

	int atleast(long long int val, int l){
		return atleast_helper(val,l,0,0,size);
	}
};
