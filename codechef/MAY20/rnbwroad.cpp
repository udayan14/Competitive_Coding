...
// All the contents of the algebra namespace have been copied from https://cp-algorithms.com/algebra/polynomial.html
// My only code is 
...
signed main() {
    ...
    polyn f = kmul(begin(x),end(x));
    polyn df = f.deriv();

    vector<base> lagrange = df.eval(x);

    for(auto& term : lagrange)
        term = term.inv();
    if(m<100000){
        polyn Q = kmul(begin(x_inv),end(x_inv));
        polyn inv = Q.inv(m+1);
        bool pos = n%2;
        base cons(-1);
        while(q--){
