...
/*All the code inside namespace algebra has been copied from https://github.com/e-maxx-eng/e-maxx-eng-aux/blob/master/src/polynomial.cpp
 * This code is part of this famous tutorial on polynomial algebra : https://cp-algorithms.com/algebra/polynomial.html 
 * I use this code mainly for interpolation and pointwise evaluation*/

using namespace std;
namespace algebra {
    const int inf = 1e9;
    const int magic = 610; // threshold for sizes to run the naive algo
...
const int Nmax = 1e5;
base a[Nmax];
vector<base> pts(Nmax+1);
vector<polyn> tree(4*(Nmax+1));
...
int main(){
    ...
    while(t--){
        ...
        for(int i=0 ; i<n ; i++){
            ...
            a[i] = mult * in;
            curr_sum += a[i];
            if(counter.count(curr_sum.r)) zero = true;
            counter[curr_sum.r]++;
            mult *= x;
        }
        ...
        base output = bpow(x,(n-1)*n*(n+1)/3).inv();
        ...
        polyn f = kmul(pts.begin(),pts.begin()+n+1);
        polyn df = f.deriv();
        build(tree,1,pts.begin(),pts.begin()+n+1);
        vector<base> products = df.eval(tree,1,pts.begin(),pts.begin()+n+1);
